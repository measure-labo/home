...
freq     = 50      # 周波数
ndata    = 4096    # 1周期のデータ数
ex_range = 2       # （バイポーラ電源で増幅後の）出力レンジ
amp_gain = 100     # バイポーラ電源の増幅利得
fg_tch   = 2       # トリガ信号を出力するサブチャネル番号
fg_clch  = 1       # クロック信号を出力するサブチャネル番号
...
- 例えば，振幅値2[V0p]の電圧信号を出力したいケースにおいて，ex_range引数で2[V0p]を設定しているのなら，任意波形データは振幅値1[V0p]の正弦波のデータとして生成し，ex_range引数で4[V0p]を設定しているのから，任意波形データは振幅値0.5[V0p]の正弦波のデータを生成すればよいことになります．
